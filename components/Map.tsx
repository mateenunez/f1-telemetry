...
import { PositionData, ProcessedDriver, ProcessedPositionData, ProcessedTiming } from "@/processors";
import { TrackPosition, CandidateLap, MapSector  } from "@/processors/map-processor";
import { fetchMap, createSectors, rotate, rad } from "@/processors/map-processor";
...
export default function Map({ positions, drivers, timing, circuitKey}: MapProps) {
		...
		>
			...
			{centerX && centerY && positions && drivers && (
				...
				.map((driver) => {
					...
					const tim = timing.find((t) => t.driver_number === driver.driver_number);
...
const CarDot = ({ pos, name, color, rotation, centerX, centerY, timing }: CarDotProps) => {
