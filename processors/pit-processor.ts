...
export class PitProcessor {
  ...
  processPitStopSeries(pitStopData: any): ProcessedPitStop[] {
    ...
    Object.entries(pitStopData.PitTimes).forEach(
      ([driverNumber, stops]: [string, any]) => {
  ...
  processTyreStintSeries(stintData: any): ProcessedStint[] {
    ...
    Object.entries(stintData.Stints).forEach(
      ([driverNumber, stints]: [string, any]) => {
        ...
        Object.entries(stints).forEach(([stintIndex, stint]: [string, any]) => {
          ...
          let existingStint = existingStints.find(
            (s) => s.stint_number === stintNum,
          );
          ...
          const updated: ProcessedStint = {
            ...existingStint,
            compound:
              stint.Compound !== undefined
                ? stint.Compound
                : existingStint.compound,
            is_new:
              stint.New !== undefined
                ? stint.New === "true"
                : existingStint.is_new,
            total_laps:
              stint.TotalLaps !== undefined
                ? Number.parseInt(stint.TotalLaps)
                : existingStint.total_laps,
            start_laps:
              stint.StartLaps !== undefined
                ? Number.parseInt(stint.StartLaps)
                : existingStint.start_laps,
            lap_flags:
              stint.LapFlags !== undefined
                ? Number.parseInt(stint.LapFlags)
                : existingStint.lap_flags,
            lap_time:
              stint.LapTime !== undefined
                ? stint.LapTime
                : existingStint.lap_time,
            lap_number:
              stint.LapNumber !== undefined
                ? Number.parseInt(stint.LapNumber)
                : existingStint.lap_number,
          };

          // Actualizar en el array existente
          const index = existingStints.findIndex(
            (s) => s.stint_number === stintNum,
          );
    ...
    Object.entries(timingAppData.Lines).forEach(
      ([driverNumber, data]: [string, any]) => {
        ...
        if (data.Stints) {
          ...
          Object.entries(data.Stints).forEach(
            ([stintIndex, stint]: [string, any]) => {
              const stintNum = Number.parseInt(stintIndex);
