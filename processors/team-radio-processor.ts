...
export class TeamRadioProcessor {
  ...
  processTeamRadio(teamRadioData: any): ProcessedTeamRadio {
    ...
    if (!Array.isArray(teamRadioData.Captures)) {
      ...
      const existing = this.teamRadio.captures.find(
        (c) => c.utc === objectValues[0].Utc
      );
      if (existing) {
        if (objectValues[0].Transcription)
          existing.transcription = objectValues[0].Transcription;
        if (objectValues[0].TranscriptionEs)
          existing.transcriptionEs = objectValues[0].TranscriptionEs;
        newCaptures = null;
      } else
        newCaptures = [
          {
      ...
      newCaptures = teamRadioData.Captures.map((capture: any) => {
        ...
        if (existing) {
          if (capture.Transcription)
            existing.transcription = capture.Transcription;
          if (capture.TranscriptionEs)
            existing.transcriptionEs = capture.TranscriptionEs;
          return null;
        } else
          return {
            utc: capture.Utc,
    ...
    if (newCaptures)
      this.teamRadio.captures.push(
        ...newCaptures.filter((c: any) => c !== null)
      );
