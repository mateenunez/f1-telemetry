...
export async function fetchUpcoming(): Promise<F1UpcomingResponse> {
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Error al obtener el evento siguiente: ${response.status}`,
      );
    }
...
export const sessionType = (
  ...
  let sessionType = translate ? "Gran Premio de" : "Grand Prix";
  if (sessionName.includes("Day")) {
    sessionType = translate ? "Pre-temporada" : "Pre-season";
  }
  return sessionType;
};

// Make a single transalteSession using dict
export const translateSessionType = (
  sessionType: string,
  dict: any,
): string => {
  if (!sessionType) {
...
export const translateSessionName = (
