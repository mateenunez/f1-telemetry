export type TrackStatusKey = 'AllClear' | 'Yellow' | 'Red' | 'Aborted' | 'SCDeployed' | 'VSCDeployed';
...
import type { ProcessedPosition, ProcessedSession, ProcessedTiming } from "@/processors";
...
export function getAboutToBeEliminatedDrivers(
  ...
  const currentPart = session.series.length > 0
  ...
  const activePositions = currentPositions.filter(pos => {
    const timing = driverTimings.find(t => t?.driver_number === pos.driver_number);
    return !timing?.knockedOut && !timing?.retired && !timing?.stopped;
  });

  if (activePositions.length > 5) {
    const sortedByPosition = [...activePositions].sort((a, b) => b.position - a.position);
